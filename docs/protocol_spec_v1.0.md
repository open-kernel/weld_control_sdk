...
```text
Payload:
01 00 03 0A 0F 34 08 80 0C 64 00 7D 00 00 00 18 01 02 AC 0D E0 2E 70 17 E0 2E B0 04 E8 03 D0 07 E8 03 88 13
```
...
| `34 08` | `single_cap_voltage_mv` | 2100mV = 2.1V |
| `80 0C` | `weld_disable_voltage_mv` | 3200mV = 3.2V |
...
| `70 17` | `single_cap_voltage_mv_max` | 6000mV = 6.0V |
| `E0 2E` | `weld_disable_voltage_mv_max` | 12000mV = 12.0V |
...
上位机应用一个设置页参数，发送 `CMD_SETTINGS_PROFILE = 0x40`。假设 `SEQ = 0x20`，Payload 为 `settings_apply_profile_t` 18 字节，不包含 `sdk_result_t` 头，也不包含 `limits_max`：

```text
AA 12 00 40 20 03 0A 0F 34 08 80 0C 64 00 7D 00 00 00 18 01 02 AC 0D 51
```
...
| 27 | `u16` | `setting_single_cap_voltage_mv` | 单电容限压，单位 mV；Dashboard 用于判断单电容实时电压是否越限 |
| 29 | `u8` | `setting_trigger_mode` | 触发模式：0 未设置，1 手动，2 自动 |
| 30 | `u16` | `setting_auto_delay_ms` | 自动触发延迟，单位 ms |
...
| 6 | `u16` | `voltage_cap_1_mv` | `voltage_cap_1_mv` | 电容 1 实时电压，单位 mV |
| 8 | `u16` | `voltage_cap_2_mv` | `voltage_cap_2_mv` | 电容 2 实时电压，单位 mV |
| 10 | `i8` | `temperature_capacitor_c` | `temperature_capacitor_c` | 电容温度，单位摄氏度，整数 |
| 11 | `i8` | `temperature_mos_c` | `temperature_mos_c` | MOS 温度，单位摄氏度，整数 |
...
| 值 | 名称 | 建议显示文案 | 说明 |
| ---: | --- | --- | --- |
| 0 | `WAITING_DATA` | 等待数据 | 上位机尚未收到有效运行数据 |
| 1 | `NOT_READY` | 未就绪 | 设备未满足点焊/运行条件 |
| 2 | `READY` | 运行就绪 | 设备可执行正常操作 |
| 3 | `FAULT` | 异常 | 设备进入异常状态 |
| 4 | `VOLTAGE_LOW` | 电压过低 | 储能电压低于 `weld_disable_voltage_mv`，应禁止点焊 |
...
### 9.2 `settings_runtime_profile_t`，17 字节

| 偏移 | 类型 | 字段 | 说明 |
| ---: | --- | --- | --- |
| 0 | `u16` | `target_voltage_mv` | 目标储能电压，单位 mV |
| 2 | `u16` | `single_cap_voltage_mv` | 单电容限压，单位 mV；必填 |
| 4 | `u16` | `weld_disable_voltage_mv` | 禁焊电压，单位 mV |
| 6 | `u16` | `target_current_a10` | 目标输出电流，单位 0.1A |
| 8 | `u16` | `preheat_pulse_ms10` | 预热脉冲时间，单位 0.1ms |
| 10 | `u16` | `cool_time_ms10` | 冷却时间，单位 0.1ms |
| 12 | `u16` | `main_pulse_ms10` | 主脉冲时间，单位 0.1ms |
| 14 | `u8` | `trigger_mode` | 0 未选择，1 手动，2 自动 |
| 15 | `u16` | `auto_delay_ms` | 自动触发延迟，单位 ms |

### 9.3 `settings_current_t`，34 字节
...
| 18 | `u16` | `target_voltage_mv_max` | 充电目标电压上限，单位 mV |
| 20 | `u16` | `single_cap_voltage_mv_max` | 单电容限压上限，单位 mV |
| 22 | `u16` | `weld_disable_voltage_mv_max` | 禁焊电压上限，单位 mV |
| 24 | `u16` | `target_current_a10_max` | 充电电流上限，单位 0.1A |
| 26 | `u16` | `preheat_pulse_ms10_max` | 预热脉冲时间上限，单位 0.1ms |
| 28 | `u16` | `cool_time_ms10_max` | 冷却时间上限，单位 0.1ms |
| 30 | `u16` | `main_pulse_ms10_max` | 主脉冲时间上限，单位 0.1ms |
| 32 | `u16` | `auto_delay_ms_max` | 自动触发延迟上限，单位 ms |

### 9.4 `settings_apply_profile_t`，18 字节
...
- `examples/micropython_device/main.py` 是当前 MicroPython 模拟设备参考实现。
...
| Dashboard 初始化响应 data | 32 字节 |
| Dashboard 初始化 ACK 完整帧 | 40 字节 |
| 高频 Dashboard Payload | 14 字节 |
| 高频 Dashboard 完整帧 | 20 字节 |
| Settings 当前参数 data | 34 字节 |
| Settings 当前参数 ACK 完整帧 | 42 字节 |
| Settings 提交参数 Payload | 18 字节 |
| Settings 提交参数完整帧 | 24 字节 |
